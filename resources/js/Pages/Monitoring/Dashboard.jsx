...
function FlowRow({ label, part, total, caption, accent = false }) {
    ...
    return <div className="py-5 first:pt-0 last:pb-0">
        <div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-on-surface">{label}</h3><p className="mt-1 text-xs leading-5 text-on-surface-variant">{caption}</p></div><span className={`shrink-0 rounded-lg px-2.5 py-1 font-monitoring tabular-nums text-sm font-semibold ${accent ? 'bg-orange-50 text-orange-800' : 'bg-emerald-50 text-emerald-900'}`}>{ratio === null ? '—' : `${fmt(ratio)}%`}</span></div>
...
function TrendPanel({ trend, periode, total, totalBatang, totalVolume }) {
    ...
    return <section className="rounded-2xl border border-outline-variant bg-white p-5 shadow-sm md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-monitoring text-xl font-bold">Tren tebangan</h2><p className="mt-1 text-sm text-on-surface-variant">Berdasarkan {monthly ? 'bulan' : 'tanggal'} pada catatan pohon</p></div><div className="flex rounded-xl bg-surface-container-low p-1" aria-label="Mode grafik">{[['daily', monthly ? 'Bulanan' : 'Harian'], ['cumulative', 'Kumulatif']].map(([key, label]) => <button key={key} type="button" onClick={() => setMode(key)} aria-pressed={mode === key} className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${mode === key ? 'bg-white text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}>{label}</button>)}</div></div>
        <div className="mt-5 flex flex-wrap items-baseline gap-x-2 gap-y-1"><strong className="font-monitoring text-3xl font-bold text-primary">{fmt(selected?.value)}</strong><span className="text-sm text-on-surface-variant">pohon {mode === 'daily' ? 'pada' : 'hingga'} {selected ? fmtTrendDate(selected.tanggal, periode.granularity) : 'periode ini'}</span><span className="rounded-lg bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-primary">{fmt(mode === 'daily' ? selected?.batang : selected?.cumulativeBatang)} batang</span><span className="rounded-lg bg-orange-50 px-2.5 py-1 text-xs font-semibold text-orange-800">{fmtVolume(mode === 'daily' ? selected?.volume : selected?.cumulativeVolume)} m³</span></div>
        <div className="mt-5 overflow-x-auto pb-1"><div className="flex h-48 items-end gap-1 border-b border-outline-variant" style={{ minWidth: `${Math.max(460, series.length * 18)}px` }} role="group" aria-label="Pilih tanggal pada grafik">
        ...
        <div className="mt-2 flex justify-between text-xs text-on-surface-variant"><span>{fmtTrendDate(periode.from, periode.granularity)}</span><span>{fmtTrendDate(periode.to, periode.granularity)}</span></div>
...
export default function Dashboard({ auth, summary, trend, kelompok, periode, filters, updatedAt, namaKelompok, canFilterKelompok, kelompokOptions }) {
    ...
    return <Layout user={auth.user}>
        ...
        <div className="font-monitoring space-y-8 pb-8" aria-busy={isFiltering}>
            ...
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.55fr)]"><section className="rounded-2xl border border-outline-variant bg-white p-5 shadow-sm md:p-6"><h2 className="font-monitoring text-xl font-bold">Alur pencatatan</h2><p className="mt-1 text-sm text-on-surface-variant">Perbandingan jumlah batang dalam periode terpilih</p><div className="mt-6 divide-y divide-outline-variant"><FlowRow label="Tebangan → dokumen angkutan" part={summary.batangTerdokumen} total={summary.batang} caption="Batang dari pohon yang memiliki dokumen angkutan." /><FlowRow label="Dokumen angkutan → SKSHHK" part={summary.batangSkshhkTerdokumen} total={summary.batangTerdokumen} caption="Batang dalam angkutan yang juga tercantum di SKSHHK." accent /></div><p className="mt-6 rounded-xl bg-orange-50 px-4 py-3 text-xs leading-5 text-orange-900">Persentase menunjukkan pencatatan dokumen, bukan status tiba di tujuan.</p></section><TrendPanel trend={trend} periode={periode} total={summary.pohonPeriode} totalBatang={summary.batangPeriode} totalVolume={summary.volumePeriode} /></div>
