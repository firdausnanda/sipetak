...
export default function Index({ tujuanBongkars, kelompoks, filters = {}, auth }) {
    ...
    const [tujuanBongkarToEdit, setJenisPohonToEdit] = useState(null);
    const [tujuanBongkarToDelete, setJenisPohonToDelete] = useState(null);
    ...
    const applyFilters = (overrides = {}) => {
        ...
        router.get(route('admin.tujuan_bongkars.index'), payload, { preserveState: true, preserveScroll: true });
    };
    ...
    const openEditModal = (tujuanBongkar) => {
        reset();
        clearErrors();
        setJenisPohonToEdit(tujuanBongkar);
        setData({
            kelompok_id: tujuanBongkar.kelompok_id,
            nama_tpk: tujuanBongkar.nama_tpk,
            titik_koordinat: tujuanBongkar.titik_koordinat || ''
        });
        setIsEditModalOpen(true);
    };

    const openDeleteModal = (tujuanBongkar) => {
        setJenisPohonToDelete(tujuanBongkar);
    ...
    const handleCreateSubmit = (e) => {
        e.preventDefault();
        post(route('admin.tujuan_bongkars.store'), {
            onSuccess: () => {
    ...
    const handleEditSubmit = (e) => {
        e.preventDefault();
        put(route('admin.tujuan_bongkars.update', tujuanBongkarToEdit.id), {
            onSuccess: () => {
    ...
    const handleDeleteSubmit = (e) => {
        e.preventDefault();
        destroy(route('admin.tujuan_bongkars.destroy', tujuanBongkarToDelete.id), {
            onSuccess: () => {
    ...
    return (
        <AdminLayout>
            <Head title="SIPETAK Admin - Master Tujuan Bongkar" />
            
            {/* Header Section */}
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                <div>
                    <h2 className="font-display text-display text-primary mb-2">Master Tujuan Bongkar</h2>
                    <p className="font-body-md text-body-md text-on-surface-variant">Kelola daftar tujuan bongkar yang tersedia.</p>
                </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                ...
                <div className="relative w-full sm:w-72 flex items-center">
                    ...
                    <input 
                        ...
                        placeholder="Cari Nama Tujuan Bongkar..."
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        <thead className="bg-surface-container-low border-b border-outline-variant sticky top-0 z-10">
                            <tr>
                                <th className="py-4 px-4 text-xs font-bold uppercase tracking-wider text-on-surface-variant cursor-pointer hover:bg-surface-container select-none" onClick={() => handleSort('nama_tpk')}>
                                    <div className="flex items-center">Nama Tujuan Bongkar <SortIcon field="nama_tpk" /></div>
                                </th>
                                <th className="py-4 px-4 text-xs font-bold uppercase tracking-wider text-on-surface-variant cursor-pointer hover:bg-surface-container select-none" onClick={() => handleSort('titik_koordinat')}>
                                    <div className="flex items-center">Titik Koordinat <SortIcon field="titik_koordinat" /></div>
                                </th>
                        ...
                        <tbody className="font-body-md text-body-md text-[#1B4332] divide-y divide-outline-variant">
                            {tujuanBongkars.data.length === 0 ? (
                                ...
                                tujuanBongkars.data.map((tujuanBongkar) => (
                                    <tr key={tujuanBongkar.id} className="even:bg-surface/30 odd:bg-surface-container-lowest hover:bg-surface-container transition-colors">
                                        <td className="py-4 px-4 font-bold text-sm text-primary">{tujuanBongkar.nama_tpk}</td>
                                        <td className="py-4 px-4 text-sm text-on-surface-variant">{tujuanBongkar.titik_koordinat || '-'}</td>
                                        <td className="py-4 px-4 text-sm text-on-surface-variant">
                                            {tujuanBongkar.kelompok?.nama_kelompok || '-'}
                                        </td>
                                        <td className="py-4 px-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <button onClick={() => openEditModal(tujuanBongkar)} className="p-2 text-[#FB8500] hover:bg-surface-container-low rounded-lg transition-colors">
                                                    <Edit className="w-4 h-4" />
                                                </button>
                                                <button 
                                                    onClick={() => openDeleteModal(tujuanBongkar)} 
                ...
                {tujuanBongkars.last_page > 1 && (
                    <div className="border-t border-outline-variant p-4 flex flex-col md:flex-row items-center justify-between bg-surface-container-lowest gap-4">
                        <span className="text-sm text-on-surface-variant font-body-md">
                            Menampilkan {tujuanBongkars.from || 0}-{tujuanBongkars.to || 0} dari {tujuanBongkars.total} data
                        </span>
                        <div className="flex gap-2 flex-wrap">
                            {tujuanBongkars.links.map((link, k) => (
                                <Link
                                    key={k}
                                    href={link.url || '#'}
                ...
                <div className="p-6">
                    ...
                    <form onSubmit={handleCreateSubmit}>
                        <div className="space-y-4">
                            ...
                            <div>
                                <InputLabel htmlFor="nama_tpk" value="Nama Tujuan Bongkar" />
                                <TextInput
                                    id="nama_tpk"
                                    type="text"
                                    name="nama_tpk"
                                    ...
                                    onChange={(e) => setData('nama_tpk', e.target.value)}
                                    placeholder="Contoh: TPK A"
                                    required
                                />
                                <InputError message={errors.nama_tpk} className="mt-2" />
                            </div>
                            
                            <div>
                                <InputLabel htmlFor="titik_koordinat" value="Titik Koordinat (Opsional)" />
                                <TextInput
                                    id="titik_koordinat"
                                    type="text"
                                    ...
                                    className="mt-1 block w-full"
                                    onChange={(e) => setData('titik_koordinat', e.target.value)}
                                    placeholder="Contoh: -6.200000, 106.816666"
                                />
                                <InputError message={errors.titik_koordinat} className="mt-2" />
                            </div>
                        </div>
                ...
                <div className="p-6">
                    ...
                    <form onSubmit={handleEditSubmit}>
                        <div className="space-y-4">
                            ...
                            <div>
                                <InputLabel htmlFor="edit_nama_tpk" value="Nama Tujuan Bongkar" />
                                <TextInput
                                    id="edit_nama_tpk"
                                    type="text"
                                    name="nama_tpk"
                            ...
                            <div>
                                <InputLabel htmlFor="edit_titik_koordinat" value="Titik Koordinat (Opsional)" />
                                <TextInput
            ...
            <Modal show={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} maxWidth="sm">
                <div className="p-6">
                    ...
                    <p className="mb-6 text-on-surface-variant">
                        Apakah Anda yakin ingin menghapus tujuan bongkar <strong>{tujuanBongkarToDelete?.nama_tpk}</strong>? Tindakan ini tidak dapat dibatalkan.
                    </p>
